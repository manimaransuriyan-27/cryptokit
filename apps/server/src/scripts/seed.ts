import { PrismaClient } from '@/generated/prisma'; // Adjust path if using custom generated output
...
async function main() {
    ...
    if (!email || !password) {
      logger.error('Seeding aborted: SUPERADMIN_EMAIL or SUPERADMIN_PASSWORD is missing in your environment configuration.');
      process.exit(1);
    }
    ...
    if (existingUser) {
      logger.warn(`Super Admin with email [${email}] already exists. Skipping allocation.`);
      return;
    }
  
    logger.info('Generating credential hashes for secure user creation...', { email });
    ...
    logger.info('Database successfully bootstrapped with Super Admin account privileges.', {
      accountEmail: email,
