...
import { LOGIN_OTP_RESPONSE_CODE, RESEND_LOGIN_OTP_RESPONSE_CODE } from '@/lib/response-codes';
...
export const LOGIN_OTP_HANDLERS: LoginOtpHandlersPayloadType = {
  OTP_VERIFIED: async (_response, ctx) => {
    ...
    try {
      ...
      ctx.navigate('/auth/gz/login', { replace: true });
    } finally {
...
export const LOGIN_OTP_ERROR_HANDLERS: ResponseCodeHandlerMap<LoginOtpErrorHandlersPayloadType> =
{
  ACCOUNT_BANNED: (response, ctx) => {
    ctx.navigate('/auth/gz/login/feedback', {
      replace: true,
...
export type ResendLoginOtpSuccessHandlersPayloadType = {
  OTP_RESENT: SuccessHandler<ResendLoginOtpSuccessResponse>;
}

export type ResendLoginOtpErrorHandlersPayloadType = ErrorHandlersPayloadType<
  typeof RESEND_LOGIN_OTP_RESPONSE_CODE,
  ResendLoginOtpErrorKeys
>;

export const RESEND_LOGIN_OTP_ERROR_HANDLERS: ResponseCodeHandlerMap<ResendLoginOtpErrorHandlersPayloadType> =
{
  INVALID_OTP_SESSION: (_response, ctx) => {
    ctx.notification.error(
      'Error',
      'The verification code session is invalid. Please request a new code.'
    );
  },
  USER_NOT_FOUND: (_response, ctx) => {
    ctx.navigate('/auth/gz/login', { replace: true });
  },
  TWO_FACTOR_NOT_ENABLED: (_response, ctx) => {
    ctx.navigate('/auth/gz/login', { replace: true });
  },
  OTP_RATE_LIMIT_EXCEEDED: (_response, ctx) => {
    ctx.notification.error(
      'Error',
      'Too many OTP requests. Please wait a few minutes before trying again.'
    );
...
export const RESEND_LOGIN_OTP_SUCCESS_HANDLERS: ResendLoginOtpSuccessHandlersPayloadType = {
  OTP_RESENT: (_response, ctx) => {
